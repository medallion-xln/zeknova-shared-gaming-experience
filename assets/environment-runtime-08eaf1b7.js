  ...
  async function loadAsset(assetKey) {
    ...
    if (isTreeAsset) {
      ...
      for (const tree of harvestableTrees) {
        ...
        instance.userData.treeVariant = variant;
        const fallback = tree.getObjectByName('fallbackAlienTree');
        if (fallback) fallback.visible = false;
        tree.add(instance);
        treeModels.push({ root: tree, model: instance, fallback });
        attachedTrees += 1;
      }

      onAssetLoaded?.(assetKey, assetPlacements.length + attachedTrees);
    } else {
  ...
  return {
    ...
    update(playerPosition) {
      ...
      for (const entry of treeModels) {
        ...
        const showHighDetail = entry.root.visible && distance <= 52;
        entry.model.visible = showHighDetail;
        if (entry.fallback) entry.fallback.visible = entry.root.visible && !showHighDetail;
      }
