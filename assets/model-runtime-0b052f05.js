...
export async function loadLieutenantModels() {
  ...
  return {
    ...
    update(delta, moving, sprinting) {
      ...
      if (moving) {
        mechGaitPhase += delta * (sprinting ? 9.5 : 6.8);
        mech.position.x = Math.sin(mechGaitPhase) * 0.035;
        mech.position.y = -0.7;
        mech.rotation.x = Math.cos(mechGaitPhase * 2) * 0.012;
        mech.rotation.z = Math.sin(mechGaitPhase) * 0.032;
      } else {
        mech.position.x *= Math.max(0, 1 - delta * 10);
        mech.position.y = -0.7;
        mech.rotation.x *= Math.max(0, 1 - delta * 10);
        mech.rotation.z *= Math.max(0, 1 - delta * 10);
      }
    },
    dispose() {
      mixer.stopAllAction();
