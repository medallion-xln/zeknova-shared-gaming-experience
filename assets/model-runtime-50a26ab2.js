...
export async function loadLieutenantModels() {
  await MeshoptDecoder.ready;
  const [astronautGltf, mechGltf, runningMechGltf] = await Promise.all([
  ...
  const mechMixer = new AnimationMixer(mechGltf.scene);
  const walkAction = mechGltf.animations[0]
    ? mechMixer.clipAction(mechGltf.animations[0])
    : null;
  const runAction = runningMechGltf.animations[0]
    ? mechMixer.clipAction(runningMechGltf.animations[0])
    : null;
  let activeMechMotion = 'idle';
  ...
  return {
    ...
    update(delta, moving, sprinting) {
      ...
      const nextMechMotion = moving ? (sprinting ? 'run' : 'walk') : 'idle';
      if (nextMechMotion !== activeMechMotion) {
        mechMixer.stopAllAction();
        const nextAction = nextMechMotion === 'run' ? runAction : walkAction;
        nextAction?.reset().play();
        activeMechMotion = nextMechMotion;
      }
      moving && mechMixer.update(delta);
    },
    dispose() {
      mixer.stopAllAction();
      mechMixer.stopAllAction();
